...
public:
    ...
    //连接
    bool connect(const char* host="localhost", const char* user="billions",
                 const char* password="123456", const char* database="fleaMarket2"){
    ...
    bool execute(const string &SqlSentence){
        ...
        if(mysql_query(this->p, str)){
            cout << "without return@" << endl;
            cout << str << endl;
	    cout << "mysql errno:" << mysql_errno(this->p) << endl;
